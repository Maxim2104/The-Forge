...
local Fossilizable = Class(function(self, inst)
    ...
    self.inst:AddTag("fossilizable")
end)

function Fossilizable:OnRemoveFromEntity()
    self.inst:RemoveEventCallback("attacked", OnAttacked)
    self.inst:RemoveTag("fossilizable")
...
function Fossilizable:SetResistance(resist)
    self.resistance = resist
end

function Fossilizable:SetDefaultWearOffTime(wearofftime)
    self.wearofftime = wearofftime
end

function Fossilizable:AddShatterFX(prefab, offset, followsymbol)
    table.insert(self.fxdata, { prefab = prefab, x = offset.x, y = offset.y, z = offset.z, follow = followsymbol })
end

function Fossilizable:SetShatterFXLevel(level, percent)
    self.fxlevel = level
--[[
...
function Fossilizable:SpawnShatterFX()
...
function Fossilizable:IsFrozen()
    return self.state == states.FROZEN or self.state == states.THAWING
end

function Fossilizable:IsThawing()
...
function Fossilizable:AddColdness(coldness, freezetime, nofreeze)
...
function Fossilizable:StartWearingOff(wearofftime)
    if self.wearofftask ~= nil then
        self.wearofftask:Cancel()
    end
...
function Fossilizable:UpdateTint()
...
function Fossilizable:SetExtraResist(resist)
...
function Fossilizable:Freeze(freezetime)
...
function Fossilizable:Unfreeze()
...
function Fossilizable:Thaw(thawtime)
    if self:IsFrozen() and not (self.inst.components.health ~= nil and self.inst.components.health:IsDead()) then
...
function Fossilizable:Reset()
    self.state = states.NORMAL
    self.coldness = 0
    self:UpdateTint()
end

function Fossilizable:OnSave()
    return self.extraresist ~= nil
...
function Fossilizable:OnLoad(data)
