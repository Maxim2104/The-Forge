...
local states=
{
	...
	State{
		...
		timeline=
		{
			TimeEvent(12*FRAMES, function(inst) 
				...
				local dist = TUNING.LAVAARENA_TURTILLUS.RANGE + 1
				local notags = { "turtle", "shadow", "playerghost", "INLIMBO", "NOCLICK", "FX" }
				local x, y, z = inst.Transform:GetWorldPosition()
				local damage = TUNING.LAVAARENA_TURTILLUS.DAMAGE
	...
	State{
		...
		ontimeout = function(inst)
			...
			if inst.slides >= TUNING.LAVAARENA_TURTILLUS.SLIDES_ALLOWED then
				inst.sg:GoToState("slide_pst")
			else
				inst.sg:GoToState("slide")
			end
	...
	State{
        ...
        onenter = function(inst)
			inst.components.health:SetAbsorptionAmount(TUNING.LAVAARENA_TURTILLUS.SHIELDED_DAMAGE_REDUCTION)
	...
	State{
        ...
        events=
        {
			...
			EventHandler("fossilize", function(inst) inst.sg:GoToState("fossilized")  end ),
...
return StateGraph("lavaarena_turtillus", states, events, "taunt", actionhandlers)
