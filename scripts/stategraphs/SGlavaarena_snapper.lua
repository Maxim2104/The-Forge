...
local events =
{
	EventHandler("attacked", function(inst, data) 
		...
		if not inst.components.health:IsDead() 
			and (inst.sg:HasStateTag("attack") or math.random() <= 0.2) then 
	...
	EventHandler("doattack", function(inst, data) 
		...
		then 
			if inst:IsNear(data.target, TUNING.LAVAARENA_SNAPPER.ATTACK_RANGE) then
...
return StateGraph("lavaarena_snapper", states, events, "taunt", actionhandlers)
