...
local function OnActivate(inst, doer)
	doer.components.talker:Say("OH MY!.")
    for k,v in pairs (AllPlayers) do	
    end
	inst:DoTaskInTime(1.3, function()	
	TheWorld:PushEvent("allplayersspawned")
    SpawnPrefab("lavaarena_portal_activefx").Transform:SetPosition(inst.Transform:GetWorldPosition())
...
local function fn()
    ...
    inst:ListenForEvent("ms_newplayercharacterspawned", function(world, data) 
        ...
        local portfx = SpawnPrefab("lavaarena_portal_activefx")
        portfx.Transform:SetPosition(inst.Transform:GetWorldPosition())
        portfx:DoTaskInTime(2, function() portfx.AnimState:PlayAnimation("portal_pst") end)
    end, TheWorld)
